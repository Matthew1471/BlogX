...
<%
' --------------------------------------------------------------------------
'¦Introduction : Category RSS Feed.                                         ¦
'¦Purpose      : This provides an RSS feed with entries from only a certain ¦
'¦               category.                                                  ¦
'¦Used By      : Includes/NAV.asp, Users' RSS readers.                      ¦
'¦Requires     : Includes/Config.asp, Includes/RSSReplace.asp,              ¦
'¦               Includes/Cache.asp.                                        ¦
'¦Ensures      : RSS feed is generated of only a certain category.          ¦
'¦Standards    : RSS 0.92.                                                  ¦
'---------------------------------------------------------------------------
...
<rss version="0.92">
<%
 '-- We encode() this later when running the SQL query so all quotes disappear --'
...
Records.Open "SELECT RecordID, Title, Text, Category, Password, Day, Month, Year, Time, UTCTimeZoneOffset, Enclosure, LastModified FROM Data WHERE Category='" & Replace(Encode(Category)," ","%20") & "' ORDER BY RecordID DESC;",Database, 1, 1

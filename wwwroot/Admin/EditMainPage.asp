<%
' --------------------------------------------------------------------------
'¦Introduction : Edit About Me Page.                                        ¦
'¦Purpose      : Allows blog administrator to edit the about me page.       ¦
...
<% If Request.Form("Action") <> "Post" Then
 ...
 '--- Open set ---'
 Records.Open "SELECT MainText FROM Main",Database, 1, 3
...
<form id="AddEntry" action="EditMainPage.asp" method="post" onsubmit="return setVar()">
 <p>
  <input name="Action" type="hidden" value="Post"/>
  Enable The Main Page<span style="color:Red">*</span> : <input name="EnableMainPage" type="checkbox" value="True" onchange="return setVarChange()" <%If EnableMainPage = True Then Response.Write "checked=""checked"""%>/>
 </p>
 ...
 <table border="0" cellpadding="1" cellspacing="0" width="100%">
  ...
  <tr>
   <td colspan="2">
    <textarea name="Content" cols="141" rows="10" style="height:40em;width:99%;" onchange="return setVarChange()"><%=Replace(Replace(MainText,"&","&amp;"),"<","&lt;")%></textarea>
   </td>
  </tr>
 </table>
...
<%
Else
 Dim EnableMainPageRequest
 EnableMainPageRequest = Request.Form("EnableMainPage")
 If EnableMainPageRequest  = "" Then EnableMainPageRequest = False Else EnableMainPageRequest = True
 If EnableMainPageRequest <> EnableMainPage Then

 Records.CursorType = 2
 Records.LockType = 3
 Records.Open "SELECT EnableMainPage FROM Config", Database
...
End If
 ...
 Records.CursorType = 2
 Records.LockType = 3
 Records.Open "SELECT MainText, LastModified FROM Main", Database
 ...
 Response.Write "<p style=""text-align:Center"">MainPage update successful.</p>"

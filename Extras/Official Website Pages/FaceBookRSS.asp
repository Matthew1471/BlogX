...
<%
' --------------------------------------------------------------------------
'¦Introduction : Social Network RSS.                                        ¦
'¦Purpose      : This provides an RSS feed specifically excluding entries   ¦
'¦               that are stored in the "Computing" and "BlogX" categories  ¦
'¦               as most of my friends are not interested in them.          ¦
'¦Used By      : Facebook's RSS Importer, TwitterFeed.com.                  ¦
'¦Requires     : Includes/Config.asp, Includes/RSSReplace.asp,              ¦
'¦               Includes/Cache.asp.                                        ¦
'¦Ensures      : My social network friends are not bored ;-).               ¦
'¦Standards    : RSS 0.92.                                                  ¦
'---------------------------------------------------------------------------
...
<channel>
<title><%=Encode(SiteDescription)%> (Only Social Entries)</title>
...
Records.Open "SELECT RecordID, Title, Text, Category, Password, Day, Month, Year, Time, UTCTimeZoneOffset, Enclosure, LastModified FROM Data WHERE Category<>'Computing' AND Category<>'BlogX' AND Category<>'Android' ORDER BY RecordID DESC;",Database, 1, 1
...
%>
    <item>
      ...
      <title><%If Title <> "" Then Response.Write Encode(Title) Else Response.Write PubDate%> (Filtered Entries)</title>
